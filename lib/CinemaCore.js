...
let CinemaCore = {
  ...
  init() {
    return CinemaCore.timeoutPromise(
      Promise.all(CinemaCore.providers.map(provider => provider.init())),
      TIMEOUT
    ).then(() => {
  ...
  loadCity(cityName) {
    ...
    .then(isLoaded => {
      if (isLoaded) {
        ...
        return CinemaCore.timeoutPromise(loadPromises, TIMEOUT);
      }
    });
  },
  ...
  // Promise helpers
  timeoutPromise(promise, ...duration) {
    let completed = false;
    return Promise.race([
      promise,
      new Promise((resolve, reject) => {
        let durObj = moment.duration(...duration);
        let msecs = durObj.asMilliseconds();
        setTimeout(() => {
          if (!completed) {
            reject(new Error(
              `The operation timed out after ${durObj.humanize()}`
            ));
            completed = true;
          }
        }, msecs);
  ...
  cleanString(str) {
    if (!str || !str.replace) {
      throw new Error('cleanString: Not a string: ' + JSON.stringify(str));
    }
  ...
  stringToSearchPattern(needle) {
    if (!needle || !needle.replace) {
      throw new Error('searchPattern: Not a string: ' + JSON.stringify(needle));
    }
  ...
  resolveKeyword({keyword, type, allowAmbiguous}) {
    ...
    .then(keywords => {
      if (!allowAmbiguous && keywords.size === 1) {
        return [...keywords][0];
  ...
  makeScreeningQuery({
    cityName, theatreName, filmTitle, variant, date, time, timezone
  }) {
