...
class BlitzConnector extends ProviderConnector {
  ...
  loadCity(cityName) {
    return this.findTheatres({cityName}).then(cinemas => {
      let theatrePromises = [];
  ...
  loadTheatre({theatreName, theatreId, cityName}) {
    ...
    .then($ => {
      ...
      let filmPromises = filmTitles.map(filmTitle => {
        let filmObj = {filmTitle};
        return this.registerFilm(filmObj);
      });

      let allPromises = [...screeningPromises, ...filmPromises];
